...
export function computeSynthesis(
  ...
  const combinedAbsences = analyzeCombinedAbsence(counts, {
    lifePath: lifePath.number,
    psycheNum,
    destinyNum,
    strongestCell: String(Object.entries(counts).sort((a,b) => b[1]-a[1])[0]?.[0] ?? '?'),
  });
  const cellPairs = analyzeCellPairs(counts, { lifePath: lifePath.number, psycheNum });
  ...
  const karmicFateNullMeaning = karmicFateNum === null
    ? getKarmicFateNullMeaning(psycheNum, destinyNum, compoundNum, counts)
    : null;

  const compoundPersonalizedInsight = getCompoundPersonalizedInsight(
    compoundNum, compoundMeaning, reducedCompoundNum, reducedCompoundMeaning, counts, psycheNum, destinyNum);

  const personalYearCustomized = getPersonalYearCustomized(birthDay, birthMonth, counts, psycheNum, destinyNum);
  const compoundEnrichedPY = getCompoundEnrichedPersonalYear(birthDay, birthMonth);
 ...
 */
function getCompoundEnrichedPersonalYear(birthDay: number, birthMonth: number): string | null {
  const yr = new Date().getFullYear();
  const r = (n: number) => String(Math.abs(n)).split('').reduce((a: number, d: string) => a + +d, 0);
  const raw = r(birthDay) + r(birthMonth) + String(yr).split('').reduce((a: number, d: string) => a + +d, 0);
  // raw is the unreduced sum — this IS the compound number
  const compound = COMPOUND_NUMBER_MEANINGS[raw];
  if (!compound || raw <= 9) return null;
  
  const c = compound.split('\n')[0] || '';
  let intro = `\n\n═══════════════════════════════\nCOMPOUND LAYER: Your raw Personal Year calculation produced the unreduced number ${raw}. This number carries its own vibrational signature that enriches and personalizes the single-digit Personal Year meaning above.\n\n${c}\n${compound.split('\n').slice(1, 5).join('\n')}`;
...
function getPersonalYearCustomized(
  birthDay: number, birthMonth: number, counts: Record<number, number>,
  psycheNum: number, destinyNum: number
): string {
  const yr = new Date().getFullYear();
  const ySum = String(yr).split('').reduce((a: number, c: string) => a + +c, 0);
  function r(n: number): number { let v = Math.abs(n); while (v > 9) v = String(v).split('').reduce((a: number, d: string) => a + +d, 0); return v; }
  const py = r(r(birthDay) + r(birthMonth) + ySum);
