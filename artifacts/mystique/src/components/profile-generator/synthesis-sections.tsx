...
export function SynthesisNumerologySections({ synthesis, numerology }: { synthesis: SynthesisResult; numerology: NumerologyData }) {
  const { combinedAbsences, cellPairs, karmicFateNullMeaning, compoundPersonalizedInsight, personalYearCustomized } = synthesis;
  const absPatterns = combinedAbsences?.patterns || [];
  const cellPatterns = cellPairs?.patterns || [];
          ...
          <AccordionItem value="personal-year" className="rounded-xl overflow-hidden" style={{ background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.15)' }}>
            <AccordionTrigger className="px-4 py-3 hover:no-underline group">
              <div className="flex items-center gap-3 text-left">
                ...
                <div>
                  <div className="flex items-center gap-2"><span className="font-cinzel text-[0.75rem] text-violet-300">Personal Year {synthesis.personalYearAnalysis.year}</span><SynthesisBadge /></div>
            ...
            <AccordionContent className="px-4 pb-4"><AccordionContentWithPlayer text={synthesis.personalYearAnalysis.interpretation} /></AccordionContent>
          </AccordionItem>
        )}
...
export function SynthesisPsychomatrixSections({
  ...
  const { lineIntersections, planeAnalysis } = synthesis;
  ...
  const hasAnything = patterns.length > 0 || !!planeAnalysis || contradictions.length > 0 || recommendations.length > 0;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.5 }} className="mt-2">
      ...
      {(patterns.length > 0 || planeAnalysis) && (
        <>
                    ...
                    <AccordionTrigger className="px-4 py-3 hover:no-underline group">
                      <div className="flex items-center gap-3 text-left">
                        ...
                        <div>
                          ...
                          <p className="text-[0.6rem] text-slate-500 font-cinzel uppercase tracking-wider mt-0.5">Score {p.score} ({p.percentage}%) • Cells {p.cells.join('-')}</p>
                        </div>
                      </div>
                    </AccordionTrigger>
                    <AccordionContent className="px-4 pb-4"><AccordionContentWithPlayer text={p.interpretation} /></AccordionContent>
