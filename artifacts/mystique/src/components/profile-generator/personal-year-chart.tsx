...
import { getCompoundForPYN } from '../../lib/numerology/chaldean-pyn-compounds';
...
export const PersonalYearChart: React.FC<PersonalYearChartProps> = ({
      ...
      datasets: [
        {
          ...
          tension: 0.4,
          borderWidth: 5,
          pointRadius: windowedData.map((_, i) => i === currentIdxInWindow ? 14 : 8),
          pointBackgroundColor: windowedData.map((_, i) => i === currentIdxInWindow ? '#fceabb' : '#c8a84b'),
  ...
  const clampWindow = useCallback((next: number) => {
  ...
  return (
    <div className="glass-card p-6 rounded-2xl bg-[#09101e]/80">
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        >
          {canScrollRight && rightmostYear !== undefined ? (rightmostYear + 1) : ''}
      ...
      <p className="text-[0.6rem] font-cinzel text-purple-200/80 text-center mt-4 italic uppercase tracking-widest">
        Personal Years cycle 1–9 • Tap point for meaning • Swipe or drag to explore
      </p>
    </div>
  );
};
